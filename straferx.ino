...
void move(int f1, int f2, int f3, int f4, int b1, int b2, int b3, int b4, int spd) {
  digitalWrite(FL_IN1, f1); digitalWrite(FL_IN2, f2); // Front Left
  digitalWrite(FR_IN1, f3); digitalWrite(FR_IN2, f4); // Front Right
  digitalWrite(BL_IN1, b1); digitalWrite(BL_IN2, b2); // Back Left
  digitalWrite(BR_IN1, b3); digitalWrite(BR_IN2, b4); // Back Right
...
void TaskMotor(void *pvParameters) {
  ...
  for (;;) {
    ...
    switch (currentAction) {
      case 'F': move(1,0, 1,0, 1,0, 1,0, rampSpeed); break; // Forward
      case 'B': move(0,1, 0,1, 0,1, 0,1, rampSpeed); break; // Backward
      
      // TURN LEFT: Left motors Back (0,1), Right motors Forward (1,0)
      case 'L': move(0,1, 1,0, 0,1, 1,0, rampSpeed); break; 
      
      // TURN RIGHT: Left motors Forward (1,0), Right motors Back (0,1)
      case 'R': move(1,0, 0,1, 1,0, 0,1, rampSpeed); break; 
